...
namespace Opencode\Http
{

    use Closure;
    use Opencode\Support\Str;
    use Opencode\Traits\Singleton;
    ...
    class Route
    {
         ...
         */
        protected $pattern = ['/*' => '/(.*)', '/?' => '/([^\/]+)', 'int' => '/([0-9]+)', 'multiInt' => '/([0-9,]+)', 'title' => '/([a-z_-]+)', 'key' => '/([a-z0-9_]+)', 'multiKey' => '/([a-z0-9_,]+)', 'isoCode2' => '/([a-z]{2})', 'isoCode3' => '/([a-z]{3})', 'multiIsoCode2' => '/([a-z,]{2,})', 'multiIsoCode3' => '/([a-z,]{3,})'];
        private $routes = [];
        private $group = '';
        private $matchedPath = '';
         ...
         * @throws \Exception
         */
        public function route(array $method, string $uri, callable $callback, array $options = [])
        {
            if(is_array($uri)) {
                foreach($uri as $u) {
            ...
            if($options['ajaxOnly'] == false || $options['ajaxOnly'] && $this->req->ajax) {
                ...
                if($this->matched === false) {
                    // Prepare.
                    $pattern = $this->prepare(str_replace(['/?', '/*'], [$this->pattern['/?'], $this->pattern['/*']], $this->removeDuplSlash($this->group . $pattern)));
        ...
        {
            $options = array_merge(['as' => $group, 'namespace' => $group], $options);

            if(is_array($group)) {
                foreach($group as $k => $p) {
                    $this->group($p, $callback, ['as' => $options['as'][$k], 'namespace' => $options['namespace'][$k]]);
                }
                return $this;
            }
...
//        $callback = $callback->bindTo($this);
         ...
         * @throws \Exception
         */
        public function resource(string $uri, $controller, array $options = [])
        {
            $options = array_merge(['ajaxOnly' => false, 'idPattern' => ':int', 'multiIdPattern' => ':multiInt'], $options);
         ...
         * @throws \Exception
         */
        public function controller(string $uri, $controller, array$options = [])
        {
            $controller = $controller;
            if(class_exists($controller)) {
         ...
         * @return mixed
         */
        protected function namedParameters(string $uri, bool $isGroup = false)
        {
         ...
         * @throws \Exception
         */
        public function _as(string $name)
        {
...
//        }
         ...
         * @return $this
         */
        public function setGroupAs($as, bool $replace = false)
        {
        ...
        {
            if($this->groupAs == '') return $this->groupAs; else
                return $this->groupAs . '.';
        }
        ...
        public function before(string $uri, $callback)
        {
            $this->before[] = ['uri' => $uri, 'callback' => $callback];
            return $this;
        }

        public function after(string $uri, $callback)
        {
            $this->after[] = ['uri' => $uri, 'callback' => $callback];
            return $this;
        }
